...
function switch_object(state, action) {
  ...
  const parsed = action.type.split('_')
  if (parsed) {
    verb = parsed[0];
    path = findLocation(parsed[parsed.length - 1]);
  }
  if (parsed.length > 1) {
    ...
    if (verb === 'UPDATE') {
      if (Array.isArray(state[path])) {
        return {...state, [path]: state[path].map( value => {
      ...
      if (typeof state[path] === 'object') {
        return {...state, [path]: state[path].map( value => {
          if(action.where(value)) return value.assign(action.value);
          return value;
        })};
      }
    }
    if (parsed[0] + parsed[1] === 'ADDTO') {
      let temp = { ...state };
      temp[path] = [...temp[path], action.value];
      return temp;
    }
  }
